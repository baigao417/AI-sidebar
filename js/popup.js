...
const PLATFORM = (() => {
  try {
    if (IS_ELECTRON && window.electronAPI && window.electronAPI.platform) {
      return String(window.electronAPI.platform);
    }
  } catch (_) {}
  try {
    const p = String((typeof navigator !== 'undefined' && navigator.platform) ? navigator.platform : '').toLowerCase();
...
const CLICK_MOD_KEY_LABEL = IS_MAC ? 'Cmd' : 'Ctrl';
const META_KEY_LABEL = IS_MAC ? 'Cmd' : (IS_WIN ? 'Win' : 'Meta');
const AuthCheckers = {
  // ChatGPT通用认证检查
...
const PROVIDERS = {
  ...
  metaso: {
    label: '秘塔AI',
    icon: 'images/providers/metaso.png',
    baseUrl: 'https://metaso.cn',
    iframeUrl: 'https://metaso.cn/',
    authCheck: null
  },
  tingwu: {
    label: '通义听悟',
    icon: 'images/providers/tingwu.png',
    baseUrl: 'https://tingwu.aliyun.com',
    iframeUrl: 'https://tingwu.aliyun.com/home',
    authCheck: null
  }
};
...
async function loadDisabledProviders() {
  return new Promise((resolve) => {
    try {
      chrome.storage?.local.get(['disabledProviders'], (res) => {
        const arr = Array.isArray(res.disabledProviders) ? res.disabledProviders : [];
        resolve(arr);
      });
    } catch (_) {
      resolve([]);
...
async function saveDisabledProviders(list) {
  try { chrome.storage?.local.set({ disabledProviders: Array.isArray(list) ? list : [] }); } catch (_) {}
}
...
const getProviderOrder = async () => {
  return new Promise((resolve) => {
    try {
      chrome.storage?.local.get(['providerOrder', 'disabledProviders', 'customProviders'], (res) => {
        const storedOrder = Array.isArray(res?.providerOrder) ? res.providerOrder.slice() : [];
        const disabled = new Set(Array.isArray(res?.disabledProviders) ? res.disabledProviders : []);
        const customList = Array.isArray(res?.customProviders) ? res.customProviders : [];
        const customKeys = customList.map((c) => (c && c.key) ? String(c.key) : '').filter(Boolean);

        const visibleBuiltins = Object.keys(PROVIDERS).filter((k) => !disabled.has(k));
        const visibleCustom = customKeys.filter((k) => !disabled.has(k));
        const visibleAll = new Set([...visibleBuiltins, ...visibleCustom]);

        let order = storedOrder.filter((k) => visibleAll.has(k) && !disabled.has(k));
        // Append any new visible providers not in stored order
        visibleBuiltins.forEach((k) => { if (!order.includes(k)) order.push(k); });
        visibleCustom.forEach((k) => { if (!order.includes(k)) order.push(k); });

        // Persist cleaned order if needed
        const same = storedOrder.length === order.length && storedOrder.every((k, i) => k === order[i]);
        if (!same) {
...
const defaultButtonShortcuts = (() => {
  ...
  return {
    openInTab: { key: 'o', ctrl: mod.ctrl, shift: false, alt: false, meta: mod.meta },
    searchBtn: { key: 'f', ctrl: mod.ctrl, shift: true, alt: false, meta: mod.meta },
    historyBtn: { key: 'h', ctrl: mod.ctrl, shift: false, alt: false, meta: mod.meta },
    favoritesBtn: { key: 'l', ctrl: mod.ctrl, shift: false, alt: false, meta: mod.meta },
    alignBtn: { key: 'a', ctrl: mod.ctrl, shift: true, alt: false, meta: mod.meta }
  };
})();
...
const ensureFrame = async (container, key, provider) => {
  ...
  if (IS_ELECTRON) {
    ...
    // 通知主进程切换到这个 provider。
    // - 内置 provider：仅发送 key，避免重复导航导致刷新
    // - 自定义 provider：需要携带 URL（首次创建/重启后恢复），主进程会做去重避免无意义 reload
    if (window.electronAPI && window.electronAPI.switchProvider) {
      const isBuiltinProvider = !!PROVIDERS[key];
      if (isBuiltinProvider) {
        window.electronAPI.switchProvider({ key });
      } else {
        let preferred = null;
        try {
          preferred = (currentUrlByProvider && currentUrlByProvider[key]) || (await getProviderUrl(key)) || null;
        } catch (_) {
          preferred = null;
        }
        window.electronAPI.switchProvider({
          key,
          url: preferred || provider.iframeUrl || provider.baseUrl || 'about:blank'
        });
      }
    }
...
const renderProviderTabs = async (currentProviderKey) => {
  ...
  // Provider order (includes custom providers + respects disabledProviders)
  let providerOrder = [];
  try { providerOrder = await getProviderOrder(); } catch (_) { providerOrder = Object.keys(PROVIDERS); }

  // Build full provider map (built-ins + customs)
  ...
  // Filter out deleted providers (e.g., removed custom providers)
  providerOrder = providerOrder.filter((k) => !!ALL[k]);
  ...
  providerOrder.forEach((key) => {
    ...
    // 悬停提示：Windows/Linux 使用 Ctrl；macOS 使用 Cmd。
    button.title = `${cfg.label}\n\n💡 提示：\n- ${CLICK_MOD_KEY_LABEL}+点击：右侧分屏\n- ${CLICK_MOD_KEY_LABEL}+Shift+点击：开启第三屏`;
    ...
    button.addEventListener('click', async (event) => {
      ...
      if (isThirdClick) {
        // Cmd+Shift+点击：第三屏
        console.log(`[Third Screen] ${CLICK_MOD_KEY_LABEL}+Shift+Click detected for provider:`, key);
        if (!__threeScreenMode) toggleThreeScreenMode(true);
        setActiveSide('third');
        if (IS_ELECTRON && window.electronAPI?.switchThirdProvider) {
          ...
          if (__thirdCurrentProvider === key) {
            ...
            window.electronAPI.switchThirdProvider(key, url);
      ...
      if (isCommandClick) {
        // Cmd+点击：触发分屏功能
        console.log(`[Split Screen] ${CLICK_MOD_KEY_LABEL}+Click detected for provider:`, key);
        ...
        if (IS_ELECTRON && window.electronAPI?.switchProvider) {
          ...
          if (__rightCurrentProvider === key) {
            ...
            if (window.electronAPI?.focusEmbedded) {
              window.electronAPI.focusEmbedded();
            }
          } else {
            // 打开右侧分屏
            window.electronAPI.switchProvider({ key, url, side: 'right' });
...
const initializeBar = async () => {
  ...
  try {
    ...
    if (openInTabBtn) {
      ...
      getButtonShortcuts().then(shortcuts => {
        ...
        const keys = [];
        if (sc.meta) keys.push(META_KEY_LABEL);
  ...
  try {
    ...
    if (settingsBtn) {
      settingsBtn.addEventListener('click', async () => {
        ...
        const formatKey = (shortcut) => `${shortcut.meta ? (META_KEY_LABEL + '+') : ''}${shortcut.ctrl ? 'Ctrl+' : ''}${shortcut.alt ? 'Alt+' : ''}${shortcut.shift ? 'Shift+' : ''}${String(shortcut.key || '').toUpperCase()}`;
        ...
        const disabledProviders = await loadDisabledProviders();
        const disabledSet = new Set(disabledProviders);
        const customProviders = await loadCustomProviders();

        let autoLaunchSection = '';
        try {
          if (IS_ELECTRON && IS_WIN && window.electronAPI?.getAutoLaunch) {
            const res = await window.electronAPI.getAutoLaunch();
            const enabled = !!(res && res.ok && (res.settings?.openAtLogin || res.settings?.enabled));
            const hint = (res && res.ok && res.isPackaged === false)
              ? '（开发模式下可能不稳定，建议安装版）'
              : '';

            autoLaunchSection = `
              <div class="settings-section-title">Startup</div>
              <div class="provider-row-wrap">
                <div class="provider-row">
                  <label class="provider-toggle">
                    <input id="autoLaunchToggle" type="checkbox" ${enabled ? 'checked' : ''}>
                    <span class="provider-label">开机自启动</span>
                    <span class="provider-key">${hint}</span>
                  </label>
                  <span class="provider-built-in">Windows</span>
                </div>
                <div class="provider-url">启用后会随 Windows 登录后台启动（可用 Alt+Space 呼出/隐藏）。</div>
              </div>
        ...
        const providerRow = (key, cfg, isCustom) => {
          const safeKey = escapeHtml(key);
          const label = escapeHtml((cfg && cfg.label) ? cfg.label : key);
          const url = escapeHtml((cfg && (cfg.iframeUrl || cfg.baseUrl || cfg.url)) || '');
          const checked = disabledSet.has(key) ? '' : 'checked';
          const right = isCustom
            ? `<button class="provider-delete-btn" type="button" data-provider-key="${safeKey}">Delete</button>`
            : `<span class="provider-built-in">Built-in</span>`;

          return `
            <div class="provider-row-wrap" data-provider-key="${safeKey}">
              <div class="provider-row">
                <label class="provider-toggle">
                  <input type="checkbox" data-provider-key="${safeKey}" ${checked}>
                  <span class="provider-label">${label}</span>
                  <span class="provider-key">(${safeKey})</span>
                </label>
                ${right}
              </div>
              ${url ? `<div class="provider-url">${url}</div>` : ''}
            </div>
          `;
        };

        const builtinRows = Object.keys(PROVIDERS)
          .map((k) => providerRow(k, PROVIDERS[k], false))
          .join('');

        const customRows = (customProviders || [])
          .map((c) => (c && c.key) ? providerRow(String(c.key), c, true) : '')
          .join('');
        ...
        modal.innerHTML = `
          ...
          <div class="settings-modal-content">
            ...
            <div class="settings-body">
              ...
              <div id="providersList" class="providers-settings-list">
                ${builtinRows}
                ${customRows ? `<div class="providers-subtitle">Custom</div>${customRows}` : ''}
              </div>

              <div class="provider-add">
                <div class="provider-add-title">Add Provider</div>
                <div class="provider-add-row">
                  <input id="providerAddName" type="text" placeholder="Name (e.g. My AI)" />
                  <input id="providerAddUrl" type="text" placeholder="URL (e.g. https://example.com/chat)" />
                </div>
                <div class="provider-add-row" style="margin-top:8px; align-items: center;">
                  <button type="button" id="providerAddIconBtn" class="provider-icon-upload" title="Upload Icon">Upload Icon</button>
                  <img id="providerAddIconPreview" style="width:24px; height:24px; border-radius:4px; object-fit:contain; margin-left:8px; display:none; background:#f1f5f9;">
                  <button id="providerAddBtn" type="button" style="margin-left:auto">Add</button>
                </div>
                <div class="provider-add-hint">提示：支持任意 http/https 站点；登录会在应用内持久化保存。</div>
              </div>
        ...
        // In Electron, BrowserViews can cover renderer overlays (like this modal).
        ...
        try { closeBtn?.addEventListener('click', closeModal); } catch (_) {}
        try { 
        ...
        // Auto-launch toggle (Windows)
        try {
          const autoToggle = modal.querySelector('#autoLaunchToggle');
          if (autoToggle && IS_ELECTRON && window.electronAPI?.setAutoLaunch) {
            autoToggle.addEventListener('change', async () => {
              const next = !!autoToggle.checked;
              autoToggle.disabled = true;
              const res = await window.electronAPI.setAutoLaunch(next);
              autoToggle.disabled = false;
              if (!res || !res.ok) {
                autoToggle.checked = !next;
                alert('设置开机自启动失败：' + String(res && res.error ? res.error : 'unknown'));
              }
            });
          }
        } catch (_) {}

        // Providers: toggle visibility, add/remove customs
        const buildAllProviders = async () => {
          const list = await loadCustomProviders();
          const all = { ...PROVIDERS };
          (list || []).forEach((c) => {
            try { if (c && c.key) all[String(c.key)] = c; } catch (_) {}
          });
          return all;
        };
        ...
        const ensureCurrentProviderVisible = async () => {
          const order = await getProviderOrder();
          const cur = await getProvider();
          if (order.includes(cur)) return cur;

          const next = order[0] || 'chatgpt';
          await setProvider(next);
          const overridesNow = await getOverrides();
          const ALL = await buildAllProviders();
          const p = effectiveConfig(ALL, next, overridesNow);
          const container = document.getElementById('iframe');
          if (container) await ensureFrame(container, next, p);
          await renderProviderTabs(next);
          try { await updateStarButtonState(); } catch (_) {}
          return next;
        };

        // Enable/disable providers
        modal.querySelectorAll('input[type=checkbox][data-provider-key]').forEach((cb) => {
          cb.addEventListener('change', async () => {
            const key = cb.getAttribute('data-provider-key');
            if (!key) return;
            const enabled = !!cb.checked;

            let disabled = await loadDisabledProviders();
            const set = new Set(Array.isArray(disabled) ? disabled : []);
            if (enabled) set.delete(key);
            else set.add(key);
            await saveDisabledProviders(Array.from(set));

            // Keep providerOrder in sync with visibility
            let storedOrder = [];
            try { storedOrder = await chrome.storage.local.get('providerOrder').then(r => r.providerOrder || []); } catch (_) { storedOrder = []; }
            if (!Array.isArray(storedOrder)) storedOrder = [];
            if (enabled) {
              if (!storedOrder.includes(key)) storedOrder.push(key);
            } else {
              storedOrder = storedOrder.filter((k) => k !== key);
            }
            await saveProviderOrder(storedOrder);

            await ensureCurrentProviderVisible();
            await renderProviderTabs(await getProvider());
          });
        });

        // Delete custom providers
        modal.querySelectorAll('.provider-delete-btn').forEach((btn) => {
          btn.addEventListener('click', async () => {
            const key = btn.getAttribute('data-provider-key');
            if (!key) return;
            if (!confirm(`Delete custom provider "${key}"?`)) return;

            let list = await loadCustomProviders();
            list = (list || []).filter((c) => String(c && c.key) !== String(key));
            await saveCustomProviders(list);

            let disabled = await loadDisabledProviders();
            disabled = (disabled || []).filter((k) => k !== key);
            await saveDisabledProviders(disabled);
        ...
        // Add custom provider
        try {
          const addBtn = modal.querySelector('#providerAddBtn');
          const iconBtn = modal.querySelector('#providerAddIconBtn');
          const previewImg = modal.querySelector('#providerAddIconPreview');
          // Removed hidden file input
          // const fileInput = modal.querySelector('#providerAddIcon'); 
          let currentIconBase64 = '';

          // Use Electron native dialog instead of file input
          // File input in Electron sometimes causes navigation/reload on selection
          const handleFileSelect = async (e) => {
            console.log('Upload Icon clicked'); // Debug
            e.preventDefault();
            e.stopPropagation();
            
            if (window.electronAPI && window.electronAPI.selectFile) {
              console.log('Calling selectFile...'); // Debug
              // Electron mode: use native dialog
              try {
                const res = await window.electronAPI.selectFile();
                console.log('selectFile result:', res); // Debug
                if (res && res.ok && res.path) {
                  // Read file content in renderer (using local fetch or file protocol)
                  // In Electron renderer, we can't use fs directly unless nodeIntegration is on.
                  // But we can fetch('file://...') or simply use the path if webSecurity allows.
                  // Better: read it as blob -> base64
                  
                  // In this context, we might not have direct file access. 
                  // Let's try to load it into an Image and draw to canvas (standard way)
                  const imgUrl = 'file://' + res.path; // Windows paths need handling? Usually Chrome handles absolute paths with file://
                  ...
                  img.onload = () => {
                    ...
                    currentIconBase64 = dataUrl;
                    if (previewImg) {
                      previewImg.src = dataUrl;
                      previewImg.style.display = 'block';
                    }
                    if (iconBtn) iconBtn.textContent = 'Change Icon';
                  };
                  img.onerror = () => {
                ...
                alert('Error opening file dialog: ' + String(err));
              }
            } else {
              // Fallback for non-Electron (standard file input, if we kept it)
              // But we are focusing on Electron fix here.
              console.warn('window.electronAPI.selectFile not found'); // Debug
              alert('File upload not supported in this mode (API missing).');
            }
          };

          iconBtn?.addEventListener('click', handleFileSelect);

          addBtn?.addEventListener('click', async () => {
            const nameEl = modal.querySelector('#providerAddName');
            const urlEl = modal.querySelector('#providerAddUrl');
            const name = String(nameEl?.value || '').trim();
            let rawUrl = String(urlEl?.value || '').trim();

            if (!name) { alert('请输入 Provider 名称'); return; }
            if (!rawUrl) { alert('请输入 URL'); return; }

            if (!/^https?:\/\//i.test(rawUrl)) rawUrl = 'https://' + rawUrl;

            let u = null;
            try { u = new URL(rawUrl); } catch (_) { alert('URL 无效'); return; }
            if (!(u.protocol === 'http:' || u.protocol === 'https:')) { alert('只支持 http/https'); return; }
            ...
            (await loadCustomProviders()).forEach((c) => { try { if (c && c.key) existing.add(String(c.key)); } catch (_) {} });

            const base = slugify(name) || slugify(u.hostname) || 'custom';
            let key = base;
            ...
            const provider = {
              key,
              label: name,
              icon: currentIconBase64 || '', // Use the uploaded icon
              baseUrl: u.origin,
              iframeUrl: u.href,
              authCheck: null
            };
            ...
            // Don't switch automatically. Just refresh the list in the modal.
            // await setProvider(key); // Removed auto-switch
            // const overridesNow = await getOverrides();
            // const ALL = await buildAllProviders();
            // const p = effectiveConfig(ALL, key, overridesNow);
            // const container = document.getElementById('iframe');
            // if (container) await ensureFrame(container, key, p);
            // await renderProviderTabs(key);

            try { 
              if (nameEl) nameEl.value = ''; 
              if (urlEl) urlEl.value = ''; 
              // Reset icon inputs
              // if (fileInput) fileInput.value = ''; // No file input anymore
              if (previewImg) { previewImg.style.display = 'none'; previewImg.src = ''; }
              if (iconBtn) iconBtn.textContent = 'Upload Icon';
              currentIconBase64 = '';
              
              alert('Provider added successfully!');
              // Re-render the settings modal content to show the new provider
              // We need to close and reopen or dynamically append. 
              // Reopening is safer to ensure state consistency.
              reopen(); 
            } catch (_) {}
          });
        } catch (_) {}
          ...
          recordBtn.addEventListener('click', async () => {
            ...
            const handleKeyDown = async (e) => {
              ...
              e.stopPropagation();
               ...
               const newKeyDisplay = `${newShortcut.meta ? (META_KEY_LABEL + '+') : ''}${newShortcut.ctrl ? 'Ctrl+' : ''}${newShortcut.alt ? 'Alt+' : ''}${newShortcut.shift ? 'Shift+' : ''}${newShortcut.key.toUpperCase()}`;
              ...
              if (shortcutId === 'star') {
                ...
                // 更新按钮的 title
                 const keys = [];
                 if (newShortcut.meta) keys.push(META_KEY_LABEL);
  ...
  // Top button handler (3-level priority)
  try {
    const topBtn = document.getElementById('toggleTopBtn');
    if (topBtn && IS_ELECTRON && window.electronAPI?.toggleAlwaysOnTop) {
      const updateTopBtn = (state) => {
        // state: 0=Normal, 1=Floating, 2=Super
        // normalize bool to int if legacy
        const s = typeof state === 'number' ? state : (state ? 1 : 0);
        
        let bg = '';
        let color = '';
        let title = 'Window Priority: Normal';
        
        if (s === 1) { // Floating
          bg = '#3b82f6'; // Blue
          color = '#ffffff';
          title = 'Window Priority: Floating (High)';
        } else if (s === 2) { // Super
          bg = '#ec4899'; // Pink/Red
          color = '#ffffff';
          title = 'Window Priority: Screen-Saver (Super Top)';
        } else {
          // Normal
          bg = ''; // Default toolbar button bg
          color = '';
        }
        
        topBtn.style.background = bg;
        topBtn.style.color = color;
        topBtn.title = title;
        // Optional: Change border if needed, or rely on default
        if (s > 0) topBtn.style.border = '1px solid transparent';
        else topBtn.style.border = '';
      };
      ...
      window.electronAPI.onAlwaysOnTopChanged?.((state) => {
        updateTopBtn(state);
      });
