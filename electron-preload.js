...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // 开机自启动（Windows/macOS）
  getAutoLaunch: () => {
    try { return ipcRenderer.invoke('auto-launch-get'); } catch (e) { return Promise.resolve({ ok: false, error: String(e) }); }
  },
  setAutoLaunch: (enabled) => {
    try { return ipcRenderer.invoke('auto-launch-set', { enabled: !!enabled }); } catch (e) { return Promise.resolve({ ok: false, error: String(e) }); }
  },
  ...
  triggerSlashPicker: () => {
    ipcRenderer.send('trigger-slash-picker');
  },
  ...
  toggleHistory: () => { // Alias for consistency
    try { ipcRenderer.send('toggle-history-panel'); } catch (_) {}
  },
  onGetHistoryData: (cb) => {
  ...
  onHistoryPanelToggled: (callback) => {
    ipcRenderer.on('history-panel-toggled', (event, data) => callback(data));
  },
  
  // File dialog
  selectFile: () => ipcRenderer.invoke('select-file'),

  // Bridge message from BrowserViews (e.g. slash triggers, timeline events)
