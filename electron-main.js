const { app, BrowserWindow, BrowserView, globalShortcut, screen, ipcMain, shell, Tray, Menu, desktopCapturer, clipboard, dialog } = require('electron');
...
// Window Priority: 0=Normal, 1=Floating (High), 2=Screen-Saver (Super Top)
let windowPriority = 0;
...
const PROVIDERS = {
  ...
  metaso: { url: 'https://metaso.cn', partition: 'persist:metaso' },
  tingwu: { url: 'https://tingwu.aliyun.com', partition: 'persist:tingwu' }
};

function sanitizeDynamicProviderKey(key) {
  try {
...
function dynamicPartitionForProviderKey(key) {
  return 'persist:' + sanitizeDynamicProviderKey(key);
}
...
function registerDynamicProvider(key, url) {
  try {
    const k = (typeof key === 'string' && key.trim()) ? key.trim() : 'custom';
    const normalized = normalizeHttpUrl(url) || String(url || '').trim();
    if (!normalized) return null;

    if (PROVIDERS[k]) {
      // If it's already a dynamic provider, allow URL updates (keep partition stable).
      if (PROVIDERS[k] && PROVIDERS[k].dynamic && typeof normalized === 'string' && normalized) {
        PROVIDERS[k].url = PROVIDERS[k].url || normalized;
        PROVIDERS[k].partition = PROVIDERS[k].partition || dynamicPartitionForProviderKey(k);
      }
      return PROVIDERS[k];
    }

    const part = dynamicPartitionForProviderKey(k);
    PROVIDERS[k] = { url: normalized, partition: part, dynamic: true };
    console.log('[Dynamic Provider] registered:', { key: k, partition: part, url: normalized });
    return PROVIDERS[k];
  } catch (e) {
    console.warn('[Dynamic Provider] register failed:', e);
    return null;
  }
}
...
function getOrCreateBrowserView(providerKey) {
  ...
  view.webContents.on('will-navigate', (event, navigationUrl) => {
    ...
    try {
      ...
      // 允许 auth/login 等认证流在当前 View 进行，以保留 Session
      const isAuthFlow = /(auth|login|sso|identity|oauth|account)/i.test(navUrl.href) || 
                         (navUrl.hostname.includes('google.com') && currentUrl.hostname.includes('google.com')) ||
                         (navUrl.hostname.endsWith('openai.com') && currentUrl.hostname.endsWith('openai.com'));

      if (navUrl.origin !== currentUrl.origin && !isAuthFlow) {
        event.preventDefault();
  ...
  view.webContents.setWindowOpenHandler(({ url }) => {
    // 允许 auth/login 等认证流在当前 View 进行（通过 loadURL 强制在当前 View 打开，以保留 Session）
    const isAuthFlow = /(auth|login|sso|identity|oauth|account)/i.test(url) || 
                       (url.includes('openai.com') && providerKey === 'chatgpt') ||
                       (url.includes('google.com') && (providerKey === 'gemini' || providerKey === 'google'));

    if (isAuthFlow) {
      console.log('[Window Open] Detected auth flow, forcing in current view:', url);
...
ipcMain.on('switch-provider', (event, payload) => {
  try {
    ...
    // Allow dynamic/custom providers registered from the renderer.
    // Once registered, it can reuse BrowserView cache and also enables partition lookup for URL matching.
    if (providerKey && !PROVIDERS[providerKey] && url) {
      try { registerDynamicProvider(providerKey, url); } catch (_) {}
    }
    ...
    if (PROVIDERS[providerKey]) {
      ...
      // 如果提供了自定义 URL，在切换后导航到该 URL（避免重复 loadURL 导致刷新）
      if (url && currentBrowserView && currentBrowserView.webContents) {
        const targetUrl = String(url);
        // 使用 setImmediate 确保 BrowserView 已经完全添加到窗口
        setImmediate(() => {
          try {
            if (!currentBrowserView || !currentBrowserView.webContents) return;
            const curUrl = currentBrowserView.webContents.getURL();
            const baseUrl = (provider && provider.url) ? String(provider.url) : '';
            // If the view already has a non-blank page, don't force-navigate back to base URL.
            if (curUrl && curUrl !== 'about:blank' && baseUrl && targetUrl === baseUrl) return;
            if (curUrl === targetUrl) return;
            console.log('Navigating to custom URL:', targetUrl);
            currentBrowserView.webContents.loadURL(targetUrl);
          } catch (e) {
...
ipcMain.on('switch-third-provider', (event, payload) => {
  try {
    ...
    let p = key ? PROVIDERS[key] : null;
    if (!p && key && url) {
      // Allow custom providers to be opened directly on the third screen.
      p = registerDynamicProvider(key, url);
    }

    const part = (p && p.partition)
      ? p.partition
      : (key ? dynamicPartitionForProviderKey(key) : 'persist:third');
...
// Auto-launch (start with OS login)
...
ipcMain.handle('auto-launch-set', async (event, payload) => {
  try {
    const enabled = !!(payload && payload.enabled);

    if (!(process.platform === 'win32' || process.platform === 'darwin')) {
      return { ok: false, error: 'unsupported-platform', platform: process.platform };
    }

    const args = enabled ? ['--autostart'] : [];

    // On Windows this writes to the StartupApproved registry.
    // Note: This is most reliable in a packaged build (app.isPackaged === true).
...
ipcMain.on('toggle-always-on-top', (event) => {
  ...
  // Cycle priority: 0 -> 1 -> 2 -> 0
  windowPriority = (windowPriority + 1) % 3;
  
  try {
    if (windowPriority === 0) {
      // Normal: Not always on top
      mainWindow.setAlwaysOnTop(false);
      mainWindow.setVisibleOnAllWorkspaces(false);
      console.log('Window Priority: 0 (Normal)');
    } else if (windowPriority === 1) {
      // High: Floating (above normal windows)
      mainWindow.setVisibleOnAllWorkspaces(true);
      mainWindow.setAlwaysOnTop(true, 'floating');
      console.log('Window Priority: 1 (Floating/High)');
    } else if (windowPriority === 2) {
      // Super Top: Screen-saver (above full screen)
      mainWindow.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true });
      mainWindow.setAlwaysOnTop(true, 'screen-saver');
      console.log('Window Priority: 2 (Super Top/Screen-Saver)');
    }
  } catch (e) {
    console.error('Failed to set window priority:', e);
  ...
  // 通知渲染进程当前状态
  event.reply('always-on-top-changed', windowPriority);
});

// 获取当前置顶状态
ipcMain.on('get-always-on-top', (event) => {
  if (!mainWindow) {
    event.reply('always-on-top-status', 0);
    return;
  }
  event.reply('always-on-top-status', windowPriority);
});
...
app.whenReady().then(() => {
  ...
  const primaryHotkeyLabel = (process.platform === 'darwin') ? 'Option+Space' : 'Alt+Space';
  const fallbackHotkeyLabel = (process.platform === 'darwin') ? 'Cmd+Shift+Space' : 'Ctrl+Shift+Space';
  console.log(`应用已启动！按 ${primaryHotkeyLabel} 或 ${fallbackHotkeyLabel}（或 F13）呼出侧边栏`);
  console.log('');
  ...
  const gotAlign = globalShortcut.register(alignKey, async () => {
    try {
      ...
      console.error('Align (' + alignKey + ') failed:', e);
    }
  });
  if (!gotAlign) console.error('Align 快捷键注册失败:', alignKey);
  else console.log('✓ Align 快捷键已注册:', alignKey);
  
  // 首次启动：默认显示窗口；若是开机自启动则保持隐藏（可用热键/托盘呼出）
  let startHidden = false;
  try {
    const s = app.getLoginItemSettings();
    startHidden = process.argv.includes('--autostart') || !!s.wasOpenedAtLogin || !!s.wasOpenedAsHidden;
  } catch (_) {
    startHidden = process.argv.includes('--autostart');
  }
  ...
  setTimeout(() => {
    ...
    if (!startHidden) {
      ...
      console.log('[AutoLaunch] started hidden; use Alt+Space / tray to show.');
    }
